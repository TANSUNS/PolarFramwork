...
uint8_t FastLowPassFR(float32_t *Src,float32_t *Des,float32_t *firCoeffs32)
{
    ...
    STATE=malloc(sizeof(q15_t)*(SIZE+NUM-1));
    

     arm_float_to_q15(Src,temp,SIZE);//将浮点数转换成Q15的定点数
      arm_float_to_q15(firCoeffs32,FIRCOEFFS,NUM);//将浮点数转换成Q15的定点数
     Status=arm_fir_init_q15(S,NUM,FIRCOEFFS,STATE,SIZE);//初始化滤波器
